...
export default function Modal({ title, visible, onCloseHandler, children, isFooter = false }: ModalProps) {
	...
	return (
		...
		>
			<div className="bg-white box-border p-8 w-[500px] h-[300px] md:w-[335px] md:h-[230px] md:p-5 rounded">
				...
				<h3 className={`text-center font-semibold text-lg text-black mt-4 ${isFooter ? 'text-2xl' : ''}`}>{title}</h3>

				<div className="w-full mt-3 text-center text-base text-black font-light leading-8">{children}</div>
			</div>
		</div>
	);
}
