...
import {customMedia, screenDesktop} from '../../../styled-components/customMedia';
...
const LoadImageWithLQIP = ({backupImgSrc, desktopImgSrc, mobileImgSrc, altText}) => {
    ...
    return (
        <ImageContainer className={`image-container ${blur ? 'blur' : 'unblur'}`}>
            ...
            <picture>
                <source srcSet={desktopImgSrc} media={`(min-width: ${screenDesktop}px)`} />
                <img src={mobileImgSrc} alt={altText} ref={largeImg} className="real-image" />
            </picture>
        </ImageContainer>
    );
};
