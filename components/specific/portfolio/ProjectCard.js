...
const ProjectCard = ({title, slug, onClickHandler, technologies, imageUrl, lowQualityImgUrl}) => {
    return (
        <CardWrapper imageUrl={imageUrl}>
            <LoadImageWithLQIP mobileImgSrc={imageUrl} desktopImgSrc={imageUrl} altText="project-card" backupImgSrc={lowQualityImgUrl} />
            <Overlay>
                <Title>{title}</Title>
                <Description>{technologies}</Description>
                <Button onClick={() => onClickHandler(slug)} tabIndex={0} onKeyPress={() => onClickHandler(slug)}>
                    Learn More
                </Button>
            </Overlay>
        </CardWrapper>
